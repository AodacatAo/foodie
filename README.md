...
1. **菜谱** —— 小红书链接/手动内容导入，自动提炼「食材 + 步骤」，分享卡片一键发家人
2. **菜单 · 点餐** —— 上架菜谱成菜单，定价分类，手机扫码点餐（美团式），订单状态机 + 微信通知，打印版菜单 PDF

（餐厅库模块已于 2026-08 整体移除，历史记录见 ARCHITECTURE.md）
...
> UI：统一设计系统 —— 桌面玻璃导航 + 手机底部悬浮 Tabbar（SVG 图标，激活态一致）、毛玻璃角标、骨架屏加载、暖色渐变主按钮；已修复「菜谱库/草稿箱」导航双高亮。

### 🍜 菜谱库
- **手动录入**：粘贴笔记正文 → LLM 提炼食材/步骤 → 草稿确认 → 入库
- **小红书链接抓取**：链接 → Playwright 登录态抓取 → 图片 OCR → LLM 提炼 → 草稿
- **视频菜谱**：Whisper 本地语音转写 + 提炼，详情页可播视频
- **分享卡片**：详情页「📤 分享卡片」一键生成图文长图（标题/封面/食材/步骤+配图），手机长按保存发微信
- **搜索**：FTS 全文搜索（菜名/食材/步骤）+ 标签过滤 + 草稿箱

### 🍽 餐厅库（已移除，2026-08）

大众点评抓取 + 收藏/评分/距离/就餐记录模块已整体移除，后端接口/前端页面/数据模型已清空；NAS 上旧数据保留不动。

### 📋 菜单 · 点餐（模块三）
- **上架**：菜谱卡片右上角「🍽 上架 / ✓ 已上架」切换（删除移至详情页）
- **管理端**（`/#/menu`）：定价（点击内联编辑）、分类（预设热菜/凉菜/汤/主食/小吃/饮品/甜品 + 自定义）、点单记录（下单人/明细/合计/删除）、**订单状态流转**（开始制作→上菜）、生成扫码二维码、**「🖨 打印菜单」导出 A4 PDF**（双栏+二维码，可打印贴冰箱）
- **用户端**（`/#/order`，扫码直达）：美团式点餐——分类栏过滤、菜品卡片（图/名/价）、圆形 +/- 份数控件、底部购物车（已点清单/份数/总合计/清空）、填名字「✔ 下单」；**无任何管理入口**（隐藏导航、无定价、无做法预览）
- **订单进度**：下单后本页显示「已下单→制作中→已上菜」进度条并自动轮询刷新；上菜时微信通知
...
- **NAS 每日备份**：`scripts/nas_backup.sh`（部署时自动挂入 crontab，每日 04:15）——数据库用容器内 sqlite backup API 一致性快照，media/快照/模型 rsync `--link-dest` 与上一份硬链接去重（仅复制变更），连同 wechat-notify 凭据备份到**异池** `/share/ZFS19_DATA/foodie-backups`（保留 14 天，`latest/` 指向最新）
...
- [x] M1 菜谱：CRUD + FTS + 小红书抓取（OCR/Whisper）+ 草稿确认 + 分享卡片
- [x] M2 餐厅：点评搜索/推荐菜真图/坐标/距离/就餐记录/评分（~~已移除~~ 2026-08）
- [x] M3 菜单点餐：上架/定价/分类/扫码点餐/购物车/下单/微信通知/订单状态机/打印菜单
