...
- **页面细节**：列表卡片角标重排（草稿/菜单中毛玻璃小标签左上角，上架操作改图标钮右上角，桌面/手机一致）；菜单「想吃」毛玻璃胶囊 + 激活渐变；详情页步骤图圆角、meta 图标化；点餐页分类栏毛玻璃吸顶、购物车深色渐变条；微修：分类 chip、购物车/清空/上菜等操作性 emoji 全部 SVG 化（仅正文语义 emoji 保留）
