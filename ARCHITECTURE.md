...
**抓取流程**：编辑/录入时贴点评链接 → 点「🔄 从点评抓取」→ 预填店名/菜系/人均/评分/地址/封面 → 保存时封面下载到 `media/restaurants/{id}.jpg`。
...
- 通用接口：`POST /api/notify {text}`（受鉴权中间件保护）——供脚本/DSH skill 等主动触达微信；配套 DSH skill「wechat-notify」
