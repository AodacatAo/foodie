...
点餐页之后，三页同级升级（同款设计语言：渐变 Hero/横幅、玻璃胶囊、stagger 入场、状态流可视化）：
- **菜单管理页**：Hero 渐变头（毛玻璃 logo/打印/扫码按钮为玻璃胶囊），卡片交错入场、想吃切换 pop、点单记录小进度点（已下单红点脉冲 → 制作中 → 上菜全绿），删除图标化
- **菜谱详情页**：封面全幅 Hero（渐变遮罩 + 题字玻璃 chip + 标签），步骤改**竖向时间线**（渐变序号圆点+连接线+图片悬浮微放大），食材自定义勾选框（渐变打勾），草稿横幅渐变金，分享弹窗毛玻璃
- **菜谱列表页**：顶部柔和渐变横幅（菜谱库+计数）、搜索框内嵌放大镜、标签胶囊渐变激活、卡片交错入场
- 三页均为纯 CSS 动画，无新增依赖；桌面/手机共用同一套组件风格

---

## 细节修复（2026-08 · 第七批）

- **菜单分类选择器被卡片裁剪**：菜单卡片原 `overflow: hidden`（为圆角裁图）把绝对定位的分类下拉框裁掉了；修复：圆角裁切转移到 `.cover` 区，卡片放开溢出；选择器 z-index 提升 + 卡片弹出态（`cat-editing`）提升层级，保证下拉悬浮于相邻卡片之上（桌面/手机均已验证）

---

## 导入/设置页升级（2026-08 · 第八批）

- **导入页**：Hero 渐变头 + 玻璃分段切换（手动粘贴/小红书链接），表单卡片化（自动提炼角标、渐变流光 CTA 按钮），任务状态胶囊化（排队=灰点、提炼中=琥珀脉冲、成功=绿、失败=红），底部提示条；原有提交/轮询逻辑不变
- **设置页**：柔和渐变横幅，局域网访问卡片（IP 胶囊 + 复制按钮带「已复制 ✓」反馈），关于卡片（logo 横标 + 技术栈 chips + GitHub 链接 + 备份说明）
- 坑：两页原未使用 Icon 组件，升级模板引用后漏导入 → 渲染为空（图标块成纯色方块），已补导入并截图验证

---

## 打印菜单 PDF 品牌化（2026-08 · 第九批）

`menu_pdf.py` v2 重做：品牌渐变头（roundRect 裁剪 + 渐变条纹 + 白圆「食」标 + 日期/统计胶囊）、分类头品牌胶囊 + 细线、条目卡片式（圆角描边 + 菜名 + meta + 品牌价）、页脚页码、末页二维码框（扫码点餐）。细节：
- 小菜单（≤7 道）整页**单栏全宽列表**（阅读顺序自然），大菜单双栏均衡
- 分类顺序按预设（热菜/凉菜/汤/主食/小吃/饮品/甜品），自定义分类殿后（端点层排序）
- 坑：wqy-microhei 无「·」间隔号字形（渲染为三角豆腐）→ 全部换用 ASCII `|` 或 `/`
